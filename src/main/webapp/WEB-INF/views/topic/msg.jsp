...
tstrap@5.3.1/dist/js/bootstrap.bundle.min.js">
</script>
</head>
<body>
			...
			<UL>
				<c:choose>
					...
					<c:when test="${code == 'create_success'}">
						<%-- Java if --%>
						<li class='li_none'><span class="span_success">새로운 카테고리를 등록했습니다.</span></li>
					</c:when>
					...
					<c:when test="${code == 'create_fail'}">
						<%-- Java if --%>
						<LI class='li_none'><span class="span_fail">새로운 카테고리 등록에 실패했습니다.</span></LI>
					</c:when>
					...
					<c:when test="${code == 'delete_success'}">
						<%-- Java if --%>
						<LI class='li_none'><span class="span_success">카테고리 삭제에 성공했습니다.</span></LI>
					</c:when>
				...
				<li class='li_none'><br> <c:choose>
					...
					<button type='button' onclick="location.href='./create.do?fcateno=${fcateno}'" class="btn btn-primary btn-sm">새로운
						카테고리 등록</button>
					...
					<button type='button' onclick="location.href='./list_by_fcateno_grid.do?fcateno=${fcateno}'"
						class="btn btn-primary btn-sm">갤러리 목록</button></li>
			</UL>
		</fieldset>
