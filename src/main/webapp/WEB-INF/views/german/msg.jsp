			...
			<ul>
				<c:choose>
					...
					<c:when test="${param.code == 'duplicate_fail'}">
						<%-- Java if --%>
						<li class='li_none'><span class="span_fail">ID가 이미 사용중입니다. 회원 가입에 실패했습니다. <br>다시 시도해주세요.
						</span></li>
					</c:when>
					<c:when test="${param.code == 'update_success'}">
						...
						<li class='li_none'>
							<button type='button' onclick="location.href='/'" class="btn btn-info btn-sm">시작 화면</button>
							<button type='button' onclick="location.href='/german/list.do'" class="btn btn-primary btn-sm">회원 목록</button>
						</li>
					</c:when>
					...
					<c:when test="${param.code == 'passwd_update_success'}">
						...
						<li class='li_none'>
							<button type='button' onclick="location.href='/'" class="btn btn-primary btn-sm">확인</button>
						</li>
					</c:when>
				...
				<li class='li_none'><br> <c:choose>
					...
					</c:choose> <%-- <a href="./list_by_cateno.do?cateno=${param.cateno}" class="btn btn-primary">목록</a> --%> <%-- <button type='button' onclick="location.href='./list_by_cateno_search.do?cateno=${param.cateno}'" class="btn btn-primary">목록</button> --%> <%-- <button type='button' onclick="location.href='./list_by_cateno_search_paging.do?cateno=${param.cateno}'" class="btn btn-primary">목록</button> --%></li>
			</ul>
		</fieldset>
	</div>
